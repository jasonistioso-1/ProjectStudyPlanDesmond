// Central Seed Dataset for PT3 Solutions Study Plan Repository (SPR)
// Rich 6-student dataset across multiple campuses, majors, and study statuses

export const mockLocations = [
    { location_id: 1, code: 'PT3-MAIN', name: 'PT3 Solutions Main Campus (Perth)' },
    { location_id: 2, code: 'PT3-SGP', name: 'PT3 Solutions Singapore Campus' },
    { location_id: 3, code: 'PT3-DXB', name: 'PT3 Solutions Dubai Campus' },
    { location_id: 4, code: 'PT3-ONL', name: 'PT3 Solutions Online Portal' }
];

export const mockTeachingPeriods = [
    { period_id: 1, code: 'S1', name: 'Semester 1', period_type: 'semester', sequence_order: 1 },
    { period_id: 2, code: 'S2', name: 'Semester 2', period_type: 'semester', sequence_order: 2 },
    { period_id: 3, code: 'T1', name: 'Trimester 1', period_type: 'trimester', sequence_order: 1 },
    { period_id: 4, code: 'T2', name: 'Trimester 2', period_type: 'trimester', sequence_order: 2 },
    { period_id: 5, code: 'T3', name: 'Trimester 3', period_type: 'trimester', sequence_order: 3 }
];

export const mockCourses = [
    { course_id: 1, code: 'PT3-BSIT-01', name: 'Bachelor of Information Technology (Major: Software & Systems)', degree_level: 'Bachelor', total_credit_points: 72 },
    { course_id: 2, code: 'PT3-BSIT-DS02', name: 'Bachelor of Data Analytics & Business Intelligence', degree_level: 'Bachelor', total_credit_points: 72 },
    { course_id: 3, code: 'PT3-MSIT-03', name: 'Master of Information Technology (Data Analytics & Cloud)', degree_level: 'Master', total_credit_points: 48 }
];

export const mockUnits = [
    { unit_id: 1, code: 'ICT100', title: 'Transition to Computing', credit_points: 3, level: 100 },
    { unit_id: 2, code: 'ICT159', title: 'Foundations of Programming', credit_points: 3, level: 100 },
    { unit_id: 3, code: 'ICT164', title: 'Discrete Mathematics & Logic', credit_points: 3, level: 100 },
    { unit_id: 4, code: 'ICT111', title: 'Cybersecurity Principles', credit_points: 3, level: 100 },
    { unit_id: 5, code: 'ICT169', title: 'Data Communications & Networks', credit_points: 3, level: 100 },
    { unit_id: 6, code: 'ICT170', title: 'Computer Systems Architecture', credit_points: 3, level: 100 },
    { unit_id: 7, code: 'ICT167', title: 'Data Structures & Algorithms', credit_points: 3, level: 100 },
    { unit_id: 8, code: 'ICT162', title: 'Applied Linear Algebra & Statistics', credit_points: 3, level: 100 },
    { unit_id: 9, code: 'ICT171', title: 'Web & Mobile Computing Basics', credit_points: 3, level: 100 },
    { unit_id: 10, code: 'ICT172', title: 'Fundamentals of Game Design', credit_points: 3, level: 100 },
    { unit_id: 11, code: 'BSC100', title: 'Building Blocks for IT Professionals', credit_points: 3, level: 100 },
    { unit_id: 12, code: 'ICT173', title: 'Introduction to Cloud & DevOps', credit_points: 3, level: 100 },
    { unit_id: 13, code: 'ICT201', title: 'IT Project Management', credit_points: 3, level: 200 },
    { unit_id: 14, code: 'ICT202', title: 'Advanced Programming & Software Architecture', credit_points: 3, level: 200 },
    { unit_id: 15, code: 'ICT284', title: 'Systems Analysis & Design', credit_points: 3, level: 200 },
    { unit_id: 16, code: 'ICT203', title: 'Distributed Systems & Network Security', credit_points: 3, level: 200 },
    { unit_id: 17, code: 'ICT285', title: 'Database Systems', credit_points: 3, level: 200 },
    { unit_id: 18, code: 'ICT206', title: 'Web Programming & Frameworks', credit_points: 3, level: 200 },
    { unit_id: 19, code: 'ICT209', title: 'Artificial Intelligence Foundations', credit_points: 3, level: 200 },
    { unit_id: 20, code: 'ICT218', title: 'Cyber Forensics & IT', credit_points: 3, level: 200 },
    { unit_id: 21, code: 'ICT258', title: 'Interactive Media & UI/UX Design', credit_points: 3, level: 200 },
    { unit_id: 22, code: 'ICT283', title: 'Object Oriented Design Patterns', credit_points: 3, level: 200 },
    { unit_id: 23, code: 'BSC203', title: 'ICT Research & Professional Ethics', credit_points: 3, level: 200 },
    { unit_id: 24, code: 'ICT304', title: 'Software Systems Architecture & Design', credit_points: 3, level: 300 },
    { unit_id: 25, code: 'ICT302', title: 'Capstone IT Practice Project', credit_points: 3, level: 300 },
    { unit_id: 26, code: 'ICT303', title: 'Advanced Database Applications', credit_points: 3, level: 300 },
    { unit_id: 27, code: 'ICT310', title: 'Mobile Application Development', credit_points: 3, level: 300 },
    { unit_id: 28, code: 'ICT311', title: 'Cloud Computing Architecture', credit_points: 3, level: 300 },
    { unit_id: 29, code: 'ICT373', title: 'Enterprise Software Engineering', credit_points: 3, level: 300 },
    { unit_id: 30, code: 'ICT308', title: 'IT Governance & Security Management', credit_points: 3, level: 300 }
];

export const mockStudents = [
    {
        student_id: 1,
        student_number: 'PT3-2026-001',
        first_name: 'Alex',
        last_name: 'Mercer',
        email: 'alex.mercer@student.pt3solutions.edu.au',
        course_id: 1,
        course_code: 'PT3-BSIT-01',
        course_name: 'Bachelor of Information Technology (Major: Software & Systems)',
        location_id: 1,
        location_name: 'PT3 Solutions Main Campus (Perth)',
        commencement_year: 2026,
        study_status: 'active'
    },
    {
        student_id: 2,
        student_number: 'PT3-2026-002',
        first_name: 'Sarah',
        last_name: 'Chen',
        email: 's.chen@student.pt3solutions.edu.au',
        course_id: 2,
        course_code: 'PT3-BSIT-DS02',
        course_name: 'Bachelor of Data Analytics & Business Intelligence',
        location_id: 2,
        location_name: 'PT3 Solutions Singapore Campus',
        commencement_year: 2026,
        study_status: 'active'
    },
    {
        student_id: 3,
        student_number: 'PT3-2025-003',
        first_name: 'David',
        last_name: 'Tan',
        email: 'd.tan@student.pt3solutions.edu.au',
        course_id: 1,
        course_code: 'PT3-BSIT-01',
        course_name: 'Bachelor of Information Technology (Major: Software & Systems)',
        location_id: 1,
        location_name: 'PT3 Solutions Main Campus (Perth)',
        commencement_year: 2025,
        study_status: 'at-risk'
    },
    {
        student_id: 4,
        student_number: 'PT3-2026-004',
        first_name: 'Emily',
        last_name: 'Watson',
        email: 'e.watson@student.pt3solutions.edu.au',
        course_id: 3,
        course_code: 'PT3-MSIT-03',
        course_name: 'Master of Information Technology (Data Analytics & Cloud)',
        location_id: 3,
        location_name: 'PT3 Solutions Dubai Campus',
        commencement_year: 2026,
        study_status: 'part-time'
    },
    {
        student_id: 5,
        student_number: 'PT3-2024-005',
        first_name: 'Michael',
        last_name: 'Rahardjo',
        email: 'm.rahardjo@student.pt3solutions.edu.au',
        course_id: 1,
        course_code: 'PT3-BSIT-01',
        course_name: 'Bachelor of Information Technology (Major: Software & Systems)',
        location_id: 1,
        location_name: 'PT3 Solutions Main Campus (Perth)',
        commencement_year: 2024,
        study_status: 'graduating'
    },
    {
        student_id: 6,
        student_number: 'PT3-2026-006',
        first_name: 'Jessica',
        last_name: 'Taylor',
        email: 'j.taylor@student.pt3solutions.edu.au',
        course_id: 2,
        course_code: 'PT3-BSIT-DS02',
        course_name: 'Bachelor of Data Analytics & Business Intelligence',
        location_id: 1,
        location_name: 'PT3 Solutions Main Campus (Perth)',
        commencement_year: 2026,
        study_status: 'active'
    }
];

export const mockStudentHistory = [
    { history_id: 1, student_id: 1, unit_id: 1, unit_code: 'ICT100', unit_title: 'Transition to Computing', status: 'completed', grade: 'D', mark: 78.5, period_id: 1, period_code: 'S1', year_taken: 2026 },
    { history_id: 2, student_id: 1, unit_id: 2, unit_code: 'ICT159', unit_title: 'Foundations of Programming', status: 'completed', grade: 'C', mark: 68.0, period_id: 1, period_code: 'S1', year_taken: 2026 },
    { history_id: 3, student_id: 1, unit_id: 5, unit_code: 'ICT169', unit_title: 'Data Communications & Networks', status: 'attempted', grade: 'F', mark: 42.0, period_id: 1, period_code: 'S1', year_taken: 2026 },
    { history_id: 4, student_id: 1, unit_id: 3, unit_code: 'ICT164', unit_title: 'Discrete Mathematics & Logic', status: 'current', grade: null, mark: null, period_id: 2, period_code: 'S2', year_taken: 2026 },
    { history_id: 5, student_id: 1, unit_id: 4, unit_code: 'ICT111', unit_title: 'Cybersecurity Principles', status: 'current', grade: null, mark: null, period_id: 2, period_code: 'S2', year_taken: 2026 },

    // David Tan (At-Risk student history)
    { history_id: 6, student_id: 3, unit_id: 1, unit_code: 'ICT100', unit_title: 'Transition to Computing', status: 'completed', grade: 'P', mark: 55.0, period_id: 1, period_code: 'S1', year_taken: 2025 },
    { history_id: 7, student_id: 3, unit_id: 2, unit_code: 'ICT159', unit_title: 'Foundations of Programming', status: 'completed', grade: 'P', mark: 52.0, period_id: 1, period_code: 'S1', year_taken: 2025 },
    { history_id: 8, student_id: 3, unit_id: 7, unit_code: 'ICT167', unit_title: 'Data Structures & Algorithms', status: 'attempted', grade: 'F', mark: 38.0, period_id: 2, period_code: 'S2', year_taken: 2025 },

    // Michael Rahardjo (Graduating senior history)
    { history_id: 9, student_id: 5, unit_id: 1, unit_code: 'ICT100', unit_title: 'Transition to Computing', status: 'completed', grade: 'HD', mark: 86.0, period_id: 1, period_code: 'S1', year_taken: 2024 },
    { history_id: 10, student_id: 5, unit_id: 2, unit_code: 'ICT159', unit_title: 'Foundations of Programming', status: 'completed', grade: 'D', mark: 81.0, period_id: 1, period_code: 'S1', year_taken: 2024 },
    { history_id: 11, student_id: 5, unit_id: 7, unit_code: 'ICT167', unit_title: 'Data Structures & Algorithms', status: 'completed', grade: 'D', mark: 79.0, period_id: 2, period_code: 'S2', year_taken: 2024 },
    { history_id: 12, student_id: 5, unit_id: 10, unit_code: 'ICT202', unit_title: 'Advanced Programming & Software Architecture', status: 'completed', grade: 'HD', mark: 89.0, period_id: 1, period_code: 'S1', year_taken: 2025 }
];

export const mockPrerequisites = [
    { target_code: 'ICT167', prereq_code: 'ICT159' },
    { target_code: 'ICT162', prereq_code: 'ICT164' },
    { target_code: 'ICT202', prereq_code: 'ICT167' },
    { target_code: 'ICT203', prereq_code: 'ICT202' },
    { target_code: 'ICT284', prereq_code: 'ICT159' },
    { target_code: 'ICT285', prereq_code: 'ICT159' }
];

export const mockDefaultPlanUnits = [
    { unit_id: 1, code: 'ICT100', title: 'Transition to Computing', credit_points: 3, year_level: 1, period_id: 1, sequence_order: 1 },
    { unit_id: 2, code: 'ICT159', title: 'Foundations of Programming', credit_points: 3, year_level: 1, period_id: 1, sequence_order: 2 },
    { unit_id: 3, code: 'ICT164', title: 'Discrete Mathematics & Logic', credit_points: 3, year_level: 1, period_id: 1, sequence_order: 3 },
    { unit_id: 4, code: 'ICT111', title: 'Cybersecurity Principles', credit_points: 3, year_level: 1, period_id: 1, sequence_order: 4 },
    { unit_id: 5, code: 'ICT169', title: 'Data Communications & Networks', credit_points: 3, year_level: 1, period_id: 2, sequence_order: 5 },
    { unit_id: 6, code: 'ICT170', title: 'Computer Systems Architecture', credit_points: 3, year_level: 1, period_id: 2, sequence_order: 6 },
    { unit_id: 7, code: 'ICT167', title: 'Data Structures & Algorithms', credit_points: 3, year_level: 1, period_id: 2, sequence_order: 7 },
    { unit_id: 8, code: 'ICT162', title: 'Applied Linear Algebra & Statistics', credit_points: 3, year_level: 1, period_id: 2, sequence_order: 8 }
];
