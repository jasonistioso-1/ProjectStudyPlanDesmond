...
export default function DraggableUnitCard({ unit, onRemoveUnit, warning, isCompleted }) {
  ...
  // Determine level color scheme & Murdoch style category badges
  const unitLevel = Number(unit.level || (unit.code ? unit.code.replace(/[^0-9]/g, '').charAt(0) + '00' : 100));
  
  let cardBg = 'bg-blue-50/80 border-blue-200 text-blue-950 hover:bg-blue-50';
  let accentBorder = 'border-l-4 border-l-blue-600';
  let badgeClass = 'bg-blue-600 text-white font-bold';
  let badgeLabel = 'CORE';

  if (unitLevel >= 300) {
    cardBg = 'bg-purple-50/80 border-purple-200 text-purple-950 hover:bg-purple-50';
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`relative p-2.5 rounded-lg border text-xs shadow-2xs transition-all group select-none ${accentBorder} ${cardBg}`}
    >
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <div className="truncate flex items-center gap-2 min-w-0">
            <span className="font-mono font-extrabold text-slate-900 text-xs tracking-tight shrink-0 bg-white/80 px-1.5 py-0.5 rounded border border-slate-200">
              {unit.code}
            </span>
        ...
        <div className="flex items-center gap-1.5 shrink-0 font-mono">
          {isCompleted ? (
            <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-700 text-white shrink-0 shadow-2xs">
              <CheckCircle className="w-3 h-3 text-white" /> DONE
            </span>
          ) : (
          ...
          <span className="text-[11px] font-bold text-slate-700 tabular-nums bg-white/90 px-1.5 py-0.5 rounded border border-slate-200">
          ...
          {onRemoveUnit && (
            <button
              ...
              className="text-slate-400 hover:text-red-700 hover:bg-white rounded px-1.5 py-0.5 text-xs font-sans font-extrabold leading-none transition-colors border border-transparent hover:border-red-200"
              title="Remove unit"
            >
              ×
            </button>
      ...
      {/* Warning Notice */}
      {warning && (
        <div className="mt-2 p-1.5 bg-amber-200/90 border border-amber-400 rounded text-[11px] text-amber-950 flex items-center gap-1.5 font-bold">
